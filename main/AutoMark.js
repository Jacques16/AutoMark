/*
 * 
 * AutoMark.js v0.0.1 (demo|test version)
...
var isOldBrowser = (userAgent.indexOf("msie") !== -1) && (parseInt(userAgent.split("msie")[1]) < 9) ? true : false;
var isBrowserEnvironment = typeof window !== 'undefined' ? true : false;
 ...
 //Hashing method that uses Jenkins' method to digitize "ID" generated by Automark.
  function Jenkins (text) {
   ...
   if (text.length === 0) {
    ...
    var char = text.charCodeAt(i); hash = ((hash << 5) - hash) + char;
    hash = hash & hash; 
   } 
  ...
  function structure(path, iarg) {
    ...
    function next(index) {
      ...
      //index actuel
      current = path[index];
      //index actuel marquee
      currentMarker = marker(current);
      //si c'est un automark element;
      if (isAutoEValide(path[index])) {
  ...
  function setFunction(lastElement, v, i, indexOfArg, nameFrames, output) {
    ...
    function outside_config() {
      for (var _j4 in AutoMark.$__root) {
