 ...
 * Released under the MIT License.
  source: https://github.com/Jacques16/Uxia.git
 *
 */
...
(function (global, factory, construct) {
  ...
  function require() {
   //v.0.0.2
    console['warn' ? 'warn' : 'log']('tools no available');
  }
 ...
 'user strict';
    ...
    value: function value(row, _value) {
     ...
     Array.prototype[rootKey.init] = arrayRemove;
      ...
      if (type[0] === "string" || type[0] === "number") {
       ...
       var test = row.e.isproto(_value.name);
    ...
    if (on.length) {
      ...
      return !xiaEvent.hasOwnProperty(i.change('on')) ? lastElement[i] = function (target) {
       ...
       s.forEach(function (_i90) {
        Uxia[_i90] = function (callback,  time) {
         function process() {
  ...
  function application(e, output, indexOfArg, methods) {
    ...
    if (!system.off) {
      ...
      return debug.window(output, 'Uxia could not run correctly');
    }
    if (vmodel && vmodel.id) {
      //...
    }
  ...
  function run_blink(a, b) {
   return caller(a ? a : arg, b ? b : Uxia);
  }
  function rootRE(str) {
    var rgxp = /^(\$_)((?:.|\n)+)/g;
    return rgxp.exec(str);
