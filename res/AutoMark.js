    ...
    if (mark.id && getid(Jenkins(mark.id))) {
      ...
      if (!isString(this.value)) {
        ...
        //
          ...
          // console.log(mark.e)
          // return console.error('No programed yet [ ' + typeof this.value + ' ] could not be insered', this.value);
        }
    ...
    for (i in base) {
      ...
      if (base[i].page) {
        if (typeof page && typeof page === 'function') {
          ...
          return console.error('Pour utiliser cette fonctionnalité, veuiller inclure la librairie de page.js: \'https://unpkg.com/page/page.js\' dans une balise script votre page HTML.\n Ou télécharger  directement le projet sur github :www.github.com/pageJs.');
        }
      }
      if (base[i].isOnce) {
  ...
  function _runPageFrameWork($, i) {
    ...
    Object.keys(list).forEach(function(k) {
      ...
      page(list[k].path, function(v) {
        if (parsing(dump.frames)) {
          if (!dump.hasOwnProperty('data')) {
            dump.data = {};
          }
          Object.assign(dump.data, {
            $page: v,
          });

          function parseMatcher(_path18, obj, temp) {
            const segment = _path18.split('.');
            if (segment.length === 1) {
              ...
              const j = Object.keys(temp)[0];
              const l = obj[_path18][j];
              const m = obj[_path18];
              Object.keys(m).forEach(function(_k17) {
                // Page actuel
                if (_k17 === j) {
                  m[_k17].addStyle = 'display:block';
                } else {
                  m[_k17].addStyle = 'display:none';
                }
              });
              return;
            }
